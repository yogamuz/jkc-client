...
const fmtK   = (n) => n >= 1_000_000 ? `${(n / 1_000_000).toFixed(1)}jt` : n >= 1_000 ? `${(n / 1_000).toFixed(0)}rb` : String(n)
...
const MiniVBar = ({ data, colorKey = C.yellow }) => {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'flex-end', gap: '3px', height: '80px', overflowX: 'auto' }}>
      {data.map((d, i) => (
        <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '2px', minWidth: '22px', flex: '1 0 22px' }}>
          <div style={{
            width: '100%',
          ...
          <div style={{ fontSize: '0.42rem', color: C.muted, fontFamily: 'monospace', writingMode: 'vertical-rl', transform: 'rotate(180deg)', lineHeight: 1, maxHeight: '28px', overflow: 'hidden' }}>
            {d.label}
          </div>
        </div>
      ))}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
        {slices.map((s, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
            <div style={{ width: '6px', height: '6px', background: s.color, flexShrink: 0 }} />
            <span style={{ fontSize: '0.6rem', fontFamily: 'monospace', color: C.muted }}>{s.label}</span>
            <span style={{ fontSize: '0.6rem', fontFamily: 'monospace', color: C.text, marginLeft: 'auto', paddingLeft: '6px' }}>
              {s.value}
            </span>
          </div>
        ))}
...
const DashboardPage = ({ onNavigate, seasons = [] }) => {
  ...
  useEffect(() => {
    ...
    fetchSummary(activeSeason.id)
    fetchWorkerSummary(activeSeason.id)
  }, [activeSeason?.id])

  const activeRates = activeSeason?.rateHistory?.length > 0
    ? [...activeSeason.rateHistory].sort((a, b) => new Date(b.effectiveDate) - new Date(a.effectiveDate))[0]
    : null

  const STATS = [
    { label: 'TOTAL SEASON', value: loading ? '...' : (dashboard?.totalSeason ?? '—'), accent: C.yellow,  Icon: ShoppingBag },
    { label: 'TOTAL ORDER',  value: loading ? '...' : (dashboard?.totalOrder  ?? '—'), accent: C.cyan,    Icon: TrendingUp  },
    { label: 'TOTAL WORKER', value: loading ? '...' : (dashboard?.totalWorker ?? '—'), accent: C.magenta, Icon: Users       },
    { label: 'TOTAL INCOME',  value: loading ? '...' : fmtRp(dashboard?.totalOmset),    accent: C.green,   Icon: Wallet      },
    { label: 'TOTAL SALARY',   value: loading ? '...' : fmtRp(dashboard?.totalGaji),     accent: C.magenta, Icon: Users       },
    { label: 'TOTAL PROFIT', value: loading ? '...' : fmtRp(dashboard?.totalProfit),   accent: C.yellow,  Icon: DollarSign  },
  ]
  ...
  /** Status breakdown */
  const STATUS_COLORS = { DONE: C.green, PROCESS: C.cyan, PENDING: C.yellow, CANCEL: C.magenta }
  const statusSlices = (() => {
    const map = {}
    orders.forEach(o => {
    ...
    return Object.entries(map)
      .map(([label, value]) => ({ label, value, color: STATUS_COLORS[label] || C.muted }))
  ...
  return (
    <div style={{ padding: 'clamp(1rem, 4vw, 2.5rem)', background: C.bg, minHeight: '100vh', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ marginBottom: '2rem', position: 'relative', zIndex: 1 }}>
        <div style={{
          fontSize: '0.6rem', fontWeight: 700, letterSpacing: '4px',
          color: C.muted, fontFamily: "'Courier New', monospace", marginBottom: '4px',
        }}>
          // OVERVIEW
        </div>
        <h1 style={{
          fontSize: 'clamp(2rem, 8vw, 4rem)', fontWeight: 900, color: C.text, margin: 0,
      ...
      <div style={{
        ...
        {STATS.map(s => (
          <div key={s.label} style={glassCard(s.accent)}>
            ...
            <div style={{
              fontSize: typeof s.value === 'string' && s.value.startsWith('Rp') ? 'clamp(0.9rem, 3vw, 1.3rem)' : 'clamp(1.4rem, 5vw, 2.2rem)',
      ...
      <div style={{
        ...
        <div style={{
          ...
          <span style={{
            ...
            {activeRates && (
              <span style={{ color: C.muted, marginLeft: '12px', letterSpacing: '1px' }}>
                berlaku sejak {new Date(activeRates.effectiveDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}
              </span>
            )}
          </span>
          <button
            ...
            style={{
              ...
              color: C.yellow, padding: '4px 12px', cursor: 'pointer',
        ...
        {!activeRates ? (
          <div style={{
            ...
            fontFamily: 'monospace', letterSpacing: '3px', fontSize: '0.72rem',
          }}>
            ...
            <table style={{ borderCollapse: 'collapse', width: '100%', minWidth: '500px' }}>
              <thead>
                <tr style={{ borderBottom: `1px solid ${C.border}` }}>
                  {['TIER', 'HARGA JOKI RANK', 'HARGA JOKI GENDONG', 'GAJI WORKER RANK', 'GAJI WORKER GENDONG'].map(h => (
                    <th key={h} style={{
              ...
              <tbody>
                {activeRates.rates.map((r, i) => (
                  <tr key={r.tier} style={{
                    borderBottom: i < activeRates.rates.length - 1 ? `1px solid ${C.border}` : 'none',
                    background: i % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.01)',
                  }}>
                    <td style={{
                      padding: '0.6rem 1.25rem', fontFamily: "'Courier New', monospace",
                      fontWeight: 900, fontSize: '0.82rem', color: C.yellow,
                      whiteSpace: 'nowrap',
                    }}>
                      {r.tier}
                    </td>
                    <td style={{ padding: '0.6rem 1.25rem', fontFamily: 'monospace', fontSize: '0.8rem', color: C.text, whiteSpace: 'nowrap' }}>
                      {fmtRp(r.rate_store_joki)}<span style={{ color: C.muted, fontSize: '0.65rem' }}> / ⭐</span>
                    </td>
                    <td style={{ padding: '0.6rem 1.25rem', fontFamily: 'monospace', fontSize: '0.8rem', color: C.text, whiteSpace: 'nowrap' }}>
                      {fmtRp(r.rate_store_jokgen)}<span style={{ color: C.muted, fontSize: '0.65rem' }}> / ⭐</span>
                    </td>
                    <td style={{ padding: '0.6rem 1.25rem', fontFamily: 'monospace', fontSize: '0.8rem', color: C.cyan, whiteSpace: 'nowrap' }}>
                      {fmtRp(r.rate_worker_joki)}<span style={{ color: C.muted, fontSize: '0.65rem' }}> / ⭐</span>
                    </td>
                    <td style={{ padding: '0.6rem 1.25rem', fontFamily: 'monospace', fontSize: '0.8rem', color: C.cyan, whiteSpace: 'nowrap' }}>
                      {fmtRp(r.rate_worker_jokgen)}<span style={{ color: C.muted, fontSize: '0.65rem' }}> / ⭐</span>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        ...
        <div style={{
          ...
          <div style={{
            fontSize: '0.6rem', fontWeight: 900, letterSpacing: '3px',
            color: C.cyan, fontFamily: "'Courier New', monospace",
          ...
          <button
            ...
            style={{
              ...
              color: C.cyan, padding: '4px 14px', cursor: 'pointer',
              fontSize: '0.6rem', fontWeight: 900, letterSpacing: '2px',
              fontFamily: "'Courier New', monospace", flexShrink: 0,
            }}
        ...
        <div style={{
          ...
          {/* Trend 14 hari */}
          <div style={{ background: C.panel, border: `1px solid ${C.border}`, padding: '1.1rem 1.25rem' }}>
            <div style={{ fontSize: '0.55rem', fontWeight: 900, letterSpacing: '2.5px', color: C.yellow, fontFamily: "'Courier New', monospace", marginBottom: '0.75rem' }}>
              TREND OMSET — 14 HARI
            </div>
            {activeSeason ? (
          ...
          {/* Status order */}
          <div style={{ background: C.panel, border: `1px solid ${C.border}`, padding: '1.1rem 1.25rem' }}>
            <div style={{ fontSize: '0.55rem', fontWeight: 900, letterSpacing: '2.5px', color: C.magenta, fontFamily: "'Courier New', monospace", marginBottom: '0.75rem' }}>
              STATUS ORDER
            </div>
          ...
          {/* Top 5 workers */}
          <div style={{ background: C.panel, border: `1px solid ${C.border}`, padding: '1.1rem 1.25rem' }}>
            <div style={{ fontSize: '0.55rem', fontWeight: 900, letterSpacing: '2.5px', color: C.cyan, fontFamily: "'Courier New', monospace", marginBottom: '0.75rem' }}>
              TOP WORKER — EARNED
            </div>
            {activeSeason && topWorkerPreview.length > 0 ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                {topWorkerPreview.map((w, i) => (
                  <div key={w.name} style={{ display: 'flex', alignItems: 'center', gap: '7px' }}>
                    <div style={{ fontSize: '0.58rem', fontFamily: 'monospace', color: i === 0 ? C.yellow : i === 1 ? '#C0C0C0' : i === 2 ? '#CD7F32' : C.muted, width: '14px', flexShrink: 0, fontWeight: 900 }}>
                      {i + 1}
                    </div>
                    ...
                    <div style={{ flex: 1, background: '#0A0A0C', height: '12px', position: 'relative' }}>
                      <div style={{
                        ...
                        background: i === 0 ? C.yellow : i === 1 ? '#C0C0C0' : i === 2 ? '#CD7F32' : C.muted,
                        minWidth: w.totalEarned > 0 ? '2px' : 0,
                      }} />
                    </div>
                    <div style={{ fontSize: '0.58rem', fontFamily: 'monospace', color: C.text, width: '60px', flexShrink: 0, textAlign: 'right' }}>
                      {fmtK(w.totalEarned)}
                    </div>
                  </div>
                ))}
          ...
          {/* Summary profit margin mini */}
          <div style={{ background: C.panel, border: `1px solid ${C.border}`, padding: '1.1rem 1.25rem' }}>
            <div style={{ fontSize: '0.55rem', fontWeight: 900, letterSpacing: '2.5px', color: C.green, fontFamily: "'Courier New', monospace", marginBottom: '0.75rem' }}>
              MARGIN SEASON INI
            </div>
            {activeSeason && summary && summary.totalOmset > 0 ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {[
                  ...
                  { label: 'GAJI WORKER', value: summary.totalGajiWorker, color: C.magenta },
                  { label: 'PROFIT',      value: summary.profitBersih,    color: C.green   },
                ].map(row => (
                  <div key={row.label}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '2px' }}>
                      <span style={{ fontSize: '0.58rem', fontFamily: 'monospace', color: C.muted, letterSpacing: '1px' }}>{row.label}</span>
                      <span style={{ fontSize: '0.58rem', fontFamily: 'monospace', color: row.color }}>
                    ...
                    <div style={{ fontSize: '0.6rem', fontFamily: 'monospace', color: row.color, marginTop: '1px' }}>
                      {fmtRp(row.value)}
                    </div>
                  </div>
                ))}
