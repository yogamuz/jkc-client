...
import { useWorkersColors, cell, fmtRp } from "./workers.utils";

const WorkersTable = ({ workers, loading, onSelectWorker }) => {
  const C = useWorkersColors();

  return (
    <div style={{ border: `1px solid ${C.yellow}60`, overflow: "hidden", background: C.panel, position: "relative", zIndex: 1 }}>
        ...
        <table style={{ borderCollapse: "collapse", minWidth: "600px", width: "100%" }}>
          <thead>
            <tr style={{ borderBottom: `1px solid ${C.border}` }}>
              {["#", "NAMA", "TOTAL ORDER", "UNPAID ORDER", "TOTAL EARNED", "TOTAL PAID", "BELUM DIBAYAR", ""].map((h) => (
                <th key={h} style={{ padding: "0.7rem 0.875rem", fontSize: "0.6rem", letterSpacing: "2px", textAlign: "left", color: C.yellow, borderRight: `1px solid ${C.border}`, background: C.bg, fontFamily: "'Courier New', monospace", fontWeight: 900, whiteSpace: "nowrap" }}>
                  {h}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {loading && workers.length === 0 ? (
              ...
              workers.map((w, i) => (
                <tr key={w.name} style={{ borderBottom: `1px solid ${C.border}`, background: i % 2 === 0 ? C.panel : C.rowAlt }}>
                  <td style={{ ...cell(false, C), color: C.muted, fontSize: "0.72rem" }}>{i + 1}</td>
                  <td style={{ ...cell(true, C), color: C.yellow }}>{w.name}</td>
                  <td style={cell(false, C)}>{w.totalOrders}</td>
                  <td style={{ ...cell(false, C), color: w.unpaidOrders > 0 ? C.magenta : C.muted }}>
                    {w.unpaidOrders > 0 ? (
                  ...
                  <td style={{ ...cell(false, C), color: C.text, fontWeight: 600 }}>{fmtRp(w.totalEarned)}</td>
                  <td style={{ ...cell(false, C), color: C.green, fontWeight: 600 }}>{fmtRp(w.totalPaid)}</td>
                  <td style={{ ...cell(false, C), color: w.totalUnpaid > 0 ? C.magenta : C.muted, fontWeight: 600 }}>{fmtRp(w.totalUnpaid)}</td>
