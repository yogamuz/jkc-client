  ...
  const fetchDetail = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiFetch(
        `/api/workers/${encodeURIComponent(workerName)}${params}`,
      );
      setDetail(res.data);
    } catch (err) {
  ...
  const statCards = detail
    ? [
        { label: "TOTAL ORDER", value: detail.totalOrders, accent: C.yellow },
        {
          ...
          value: fmtRp(detail.totalEarned),
          accent: C.cyan,
        },
        {
          label: "TOTAL DIBAYAR",
          value: fmtRp(detail.totalPaid),
          accent: C.green,
        },
        {
          label: "BELUM DIBAYAR",
          value: fmtRp(detail.totalUnpaid),
          accent: C.magenta,
        },
      ]
    : [];
          ...
          {["ALL", "UNPAID", "PAID"].map((f) => (
            <button
              ...
              style={{
                ...
                boxShadow:
                  paidFilter === f
                    ? `0 0 10px ${f === "UNPAID" ? C.magenta : f === "PAID" ? C.cyan : C.yellow}40`
                    : "none",
              }}
            >
              {f}
                    ...
                    <td style={{ ...cell(false, C), verticalAlign: "middle" }}>
                      <span
                        style={{
                          ...
                          fontWeight: 900,
                          letterSpacing: "1px",
                          background: h.isPaid ? C.greenBg : C.magentaBg,
